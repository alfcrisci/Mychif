...
setwd("/home/alf/Scrivania/lav_michyf/lav_PDI/adults_DON")
...
DON_data_nation_full=DON_data_nation_full[1:2,]
...
for ( i in 1:length(exp_par_DON)) {
                                  temp_PDI=sapply(rexp(10000,exp_par_DON[i]),FUN=function(x){ pdi_func(x,vol_urine =2,weight=mat_meta$weight.mean[i])})
...
df_pdi_DON$vol_urine=2

temp=rbind(df_pdi_DON)
temp_fit=rbind(df_res_DON)

file.remove("DON_PDI_stat_adulti.xls")
XLConnect::writeWorksheetToFile("DON_PDI_stat_adulti.xls",temp,"PDI")
XLConnect::writeWorksheetToFile("DON_PDI_stat_adulti.xls",temp_fit,"fit stats")
XLConnect::writeWorksheetToFile("DON_PDI_stat_adulti.xls",mat_meta,"metadati")
