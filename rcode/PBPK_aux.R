...
multi_tool <- function(par_in,
  ...
  timeinthearth=floor(60/timephysioheart) # time resolution heart and pulmonary activity
  timeint=floor(60/timestep) # time resolution physio
  ...
  CO <- par_in$CO/timeinthearth    # cardiac output (L/3sec)
  
  # source data are in min -1.
  
  Qexhale <- (0.499 * (BW^0.81))/ timeinthearth        # Ventilation mammalian (L/3sec)
 
  if (species=="chicken") { Qexhale <- ((284 * (BW^0.77))/1000)/ timeinthearth}    # Ventilation avian (L/3sec)
...
rate_kabs=function(tmax,ke,p=0.001) { x=0 ; 
x=10;
k=0
while(tmax>= k) {
  k=(log(x/ke))/(x-ke)
